...
import { IconCpu, IconRam, IconDisk, IconPlayers } from "../components/icons";
...
export function ServersPage() {
  ...
  async function power(id: string, action: string) {
    await api(`/api/servers/${id}/power`, { method: "POST", body: JSON.stringify({ action }) }).catch(
      () => {},
    );
  ...
  return (
    <>
      ...
      {showForm && (
        <section className="card">
          ...
          <form onSubmit={create} className="form">
            ...
            {!onlineNodes.length && <p className="status warn">No online nodes — connect a node first.</p>}
      ...
      {!servers.length && (
        <section className="card">
          <p className="dim">No servers yet. Click “New Server” to create one.</p>
        </section>
      )}
      ...
      <div className="server-grid">
        {servers.map((s) => {
          ...
          return (
            <article key={s.id} className="server-card">
              <div className="banner">
                <span className={`pill ${STATE_CLASS[s.state] ?? ""}`}>● {s.state}</span>
              </div>
              <div className="body">
                <div className="title">
                  <h3>
                  ...
                  <button
                    ...
                    onClick={() => power(s.id, running ? "stop" : "start")}
                  >
                    {running ? "■" : "▶"}
                  </button>
                </div>
                <div className="resource-row">
                  <Resource icon={<IconCpu size={16} />} value={`${s.cpuPercent}%`} />
                  <Resource icon={<IconRam size={16} />} value={`${s.memoryMiB}MB`} />
                  <Resource icon={<IconDisk size={16} />} value={`${(s.diskMiB / 1024).toFixed(0)}GB`} />
                  <Resource icon={<IconPlayers size={16} />} value={`:${s.primaryPort}`} />
                </div>
              </div>
            </article>
          );
